...
So we just need to test if `equals(Circle)` can accept whatever arguments can be accepted by `equals(Object)` and vice versa.  So we test `equals(Circle)` and attempt to pass `Object` and `Circle` as argument (since `equals(Object)` can accept both `Object` and `Circle`).  `Circle` is fine but it will fail to compile on `Object`.

Similarly, if we test `equals(Object)` and attempt to pass `Circle` (since `equals(Circle)` can only accept `Circle`).  There is no compilation error here. Returning to our definition, _"the arguments to `equals(Object)` cannot be passed to `equals(Circle)` as it results in compilation error"_ .

Therefore, `equals(Circle)` is more specific than `equals(Object)`.  There is also the possibility that in comparing only two arbitrary methods, we will find that none of the two methods is more specific!  For instance, given `S1` <: `T` and `S2` <: `T`, `foo(S1)` is not more specific than `foo(S2)` and `foo(S2)` is not more specific than `foo(S1)`.

Once the method is determined, the method's descriptor (return type and signature) is stored in the generated bytecode.
